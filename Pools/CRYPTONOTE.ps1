...
if (($Querymode -eq "core" ) -or ($Querymode -eq "Menu")){
		...
		if ($Wallet -ne $null -and $Wallet -ne "") {
			...
			$TRADEOGRE_Coin = $TRADEOGRE_Request | where-object { $_.$TradeOgrePair -ne $null } | Select-Object -ExpandProperty $TradeOgrePair
				...
				$CRYPTONOTE2_price = [double] $TRADEOGRE_Coin.bid * 86400 / $CRYPTONOTE2_Request.network.difficulty * $CRYPTONOTE2_Request.network.reward / $CRYPTONOTE2_Request.config.coinUnits
